...
/**
 * CertificadosDignatariosController implements the CRUD actions for Entidades model.
 */
class CertificadosDignatariosController extends Controller
{
    /**
     * @inheritdoc
     */
     ...
     public function actionView($id)
     {
       ...
       if(empty($dignatario)){
         ...
         if($a <= 0  ){
           ...
           $templateWord = new TemplateProcessor('plantillas/certificado dignatarios.docx');
           $valores = Valores::find()->all();
           $dignatarios = Dignatarios::find()->where(['id_entidad'=>$entidad['id_entidad']])->orderBy('id_cargo')->all();
           $dig = "";
           ...
           $templateWord->saveAs('Certificado diganatarios '.$entidad['nombre_entidad'].'.docx');
           header('Content-Disposition: attachment; filename=Certificado diganatarios '.$entidad['nombre_entidad'].'.docx; charset=iso-8859-1');
           echo file_get_contents('Certificado diganatarios '.$entidad['nombre_entidad'].'.docx');
